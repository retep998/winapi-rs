...
use shared::minwindef::{BOOL, BYTE, DWORD, LPARAM, LPVOID, PBOOL, ULONG};
use um::minwinbase::{LPOVERLAPPED, LPSECURITY_ATTRIBUTES};
use um::threadpoolapiset::{PTP_WIN32_IO_CALLBACK};
use um::winbase::PFIBER_CALLOUT_ROUTINE;
use um::winnls::LPNLSVERSIONINFO;
use um::winnt::{
    HANDLE, LCID, LPCSTR, LPCWCH, LPCWSTR, PCNZCH, PTP_CALLBACK_INSTANCE, PTP_CLEANUP_GROUP, PTP_IO,
    PTP_POOL, PTP_TIMER, PTP_WAIT, PTP_WORK, PVOID,
};
...
extern "system" {
    ...
    pub fn SetNamedPipeAttribute(
        Pipe: HANDLE,
        AttributeType: PIPE_ATTRIBUTE_TYPE,
        AttributeName: PSTR,
        AttributeValue: PVOID,
        AttributeValueLength: SIZE_T,
    ) -> BOOL;

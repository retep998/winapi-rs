...
use ctypes::{c_char, c_float, c_int, c_long, c_void};
...
use um::winnt::{HANDLE, HRESULT, LANGID, LONG, LONGLONG, LPCWSTR, LPWSTR, ULONGLONG, WCHAR};
...
interface ISpPhoneticAlphabetConverter(ISpPhoneticAlphabetConverterVtbl): IUnknown(IUnknownVtbl) {
    fn GetLangId(pLangID: *mut LANGID) -> HRESULT,
    fn SetLangId(LangID: LANGID) -> HRESULT,
    fn SAPI2UPS(
...
// TODO: Move into SPEAKFLAGS enum block?
...
pub const SPF_VOICE_MASK: i32 = SPF_ASYNC as i32 | SPF_PURGEBEFORESPEAK as i32
    ...
    | SPF_NLP_MASK as i32 | SPF_PERSIST_XML as i32 | SPF_PARSE_MASK;
...
interface ISpGrammarBuilder(ISpGrammarBuilderVtbl): IUnknown(IUnknownVtbl) {
    fn ResetGrammar(NewLanguage: LANGID) -> HRESULT,
    fn GetRule(
        pszRuleName: LPCWSTR,
        dwRuleId: DWORD,
        dwAttributes: DWORD,
...
STRUCT!{struct SPRECOGNIZERSTATUS {
    ...
    clsidEngine: CLSID,
    cLangIDs: ULONG,
    aLangID: [LANGID; SP_MAX_LANGIDS as usize],
